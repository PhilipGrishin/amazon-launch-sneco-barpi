# Claims Review

## Public Repository Rule

This repository is public. Do not include confidential costs, supplier data, private documents, personal data, credentials, or non-public commercial strategy.

## Active Scope

TM Barpi brand and product intake is the current active focus.

SnEco claims work is paused until production price, packaging, and unit economics inputs are ready.

Current active TM Barpi claims review applies only to:

- Cheese sticks 20g for dogs
- Chicken hearts 20g for dogs
- Zefir 20g for dogs
- Beze 20g for dogs
- Liver 20g for dogs

Target marketplaces:

- amazon.es
- amazon.it
- amazon.fr
- amazon.de

TM Barpi website claims must not be automatically used in Amazon listings.

## Rule

Evidence availability does not automatically mean the claim is approved for Amazon listing use. All claims must be checked against Amazon policy and EU marketplace requirements before publication.

Do not use unsupported health, safety, environmental, nutrition, award, origin, or performance claims in Amazon listing copy.

## Preliminary EU Food Information Finding

EU Regulation 1169/2011 requires food information not to mislead consumers regarding characteristics such as nature, identity, properties, composition, quantity, durability, country of origin/provenance, and method of manufacture/production.

For distance selling, required food information should be available before purchase except date of minimum durability/use-by. Mandatory food information should appear in a language easily understood by consumers in the Member State where the food is marketed.

## Status Values

- Evidence available
- Evidence needed
- Needs EU claim check
- Needs Amazon listing review
- Approved for packaging only
- Not approved for listing yet
- Potentially usable after review

## Claims Tracker

| Claim | SKU / scope | Evidence status | Compliance status | Amazon listing use status | Risk | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| From 100% Cheddar Cheese | SnEco Cheddar 28g EN | Evidence available | Needs EU claim check | Not approved for listing yet | Medium | Evidence available through composition/specification. Preferred safer listing wording: "Made from Cheddar cheese" or "Crunchy bites made from cheese". Avoid over-broad wording such as "100% pure cheese" until reviewed. |
| From 100% Gouda Cheese | SnEco Gouda 28g EN | Evidence available | Needs EU claim check | Not approved for listing yet | Medium | Evidence available through composition/specification. Preferred safer listing wording: "Made from Gouda cheese" or "Crunchy bites made from cheese". Avoid over-broad wording such as "100% pure cheese" until reviewed. |
| Produced utilizing renewable energy | SnEco Cheddar 28g EN; SnEco Gouda 28g EN | Evidence available | Needs EU claim check | Not approved for listing yet | Medium | Evidence available through solar panels. Do not make this a main listing claim at first launch; use only after evidence file and wording review. |
| SIAL Innovation Paris 2024 Grand Prix | SnEco Cheddar 28g EN; SnEco Gouda 28g EN | Evidence available | Needs Amazon listing review | Potentially usable after review | Medium | Certificate available. Potentially usable after exact certificate wording and logo/award-use rights are verified. Correct award spelling: SIAL Innovation Paris 2024 Grand Prix. |
| High protein | SnEco Cheddar 28g EN; SnEco Gouda 28g EN | Evidence available | Needs EU claim check | Potentially usable after review | Medium | EU Regulation 1924/2006 allows "high protein" where at least 20% of the energy value is provided by protein. Preliminary calculation: Cheddar 169.2 kcal from protein / 605 kcal = about 28.0%; Gouda 193.2 kcal from protein / 625 kcal = about 30.9%. Use only after nutrition value verification and Amazon listing wording review. |
| Keto friendly | SnEco Cheddar 28g EN; SnEco Gouda 28g EN | Evidence needed | Needs EU claim check | Approved for packaging only | High | Do not use in Amazon listing at first launch. Keep as packaging claim only until separate compliance decision. |
| Healthy crunchy supersnack | SnEco Cheddar 28g EN; SnEco Gouda 28g EN | Evidence needed | Needs EU claim check | Not approved for listing yet | High | Not recommended for Amazon listing. Avoid in title, bullets, A+, and backend content. |
| 18 months shelf life | SnEco Cheddar 28g EN; SnEco Gouda 28g EN | Evidence available | Needs Amazon listing review | Not approved for listing yet | Medium | Shelf life is 18 months; confirm whether and how it can be used in Amazon listing copy. |
| 100% cheese wording for Amazon listing phrasing | SnEco Cheddar 28g EN; SnEco Gouda 28g EN | Evidence available | Needs EU claim check | Not approved for listing yet | Medium | Preferred safer listing wording: "Made from Cheddar cheese", "Made from Gouda cheese", or "Crunchy bites made from cheese". Avoid over-broad wording such as "100% pure cheese" until reviewed. |

## Required Amazon Data Point

| Data point | SKU / scope | Status | Notes |
| --- | --- | --- | --- |
| Contains: Milk | SnEco Cheddar 28g EN; SnEco Gouda 28g EN | Needs Amazon listing review | Milk must be treated as a critical allergen disclosure because EU Regulation 1169/2011 lists milk and products thereof, including lactose, as an allergen category. |

## TM Barpi Amazon Compliance Notes

Do not automatically use website claims in Amazon listing.

Website claims must not be used automatically in Amazon listings. Claims related to dental health, immunity, digestion, joints, allergy/hypoallergenic, vitamins, functional benefits, or medical-style outcomes are not approved for listing use.

## TM Barpi High-Risk Claims Needing Review

| Claim area | Amazon listing use status | Risk | Notes |
| --- | --- | --- | --- |
| Dental health / cleans teeth / removes plaque | Not approved for listing yet | High | Health or functional-style claim; requires separate compliance review and evidence. |
| Immune support | Not approved for listing yet | High | Health or medical-style claim; avoid until reviewed. |
| Digestion improvement / microbiome normalization | Not approved for listing yet | High | Health or functional-style claim; avoid until reviewed. |
| Joint support | Not approved for listing yet | High | Health or functional-style claim; avoid until reviewed. |
| Hypoallergenic / allergy claims | Not approved for listing yet | High | Requires strict evidence and compliance review. |
| Functional additive / vitamin supplement claims | Not approved for listing yet | High | May change category or compliance route. |
| Human-grade implication | Not approved for listing yet | High | Avoid unless evidence and compliant wording are confirmed. |
| Space technology wording | Not approved for listing yet | High | Avoid unless evidence, accuracy, and Amazon wording are reviewed. |
| Health or medical-style claims | Not approved for listing yet | High | Avoid in title, bullets, A+, and backend content. |
| Liver nutrient / vitamin / functional benefit wording | Not approved for listing yet | High | Liver products require careful claims review to avoid health, vitamin, functional, or medical-style claims. |
| Cheese-based pet treat positioning | Not approved for listing yet | Medium | Cheese sticks may require separate compliance attention because they are cheese-based pet treats. |

## TM Barpi Preferred Amazon-Safe Language

| Preferred language | Use status | Notes |
| --- | --- | --- |
| Single ingredient | Potentially usable after review | Verify exact SKU composition first. |
| Crunchy meat treat | Potentially usable after review | Verify texture and SKU-specific wording. |
| For dogs / for cats | Potentially usable after review | Confirm intended animal type by SKU. |
| Reward / training / daily treat | Potentially usable after review | Avoid health implications. |
| Easy to portion | Potentially usable after review | Verify format and pack presentation. |
| No unnecessary additives | Potentially usable after review | Confirm final formulation and compliant wording. |
| Vacuum-dried using SNECO technology | Potentially usable after review | Confirm technology evidence and wording. |
| Lab-tested / quality-controlled | Potentially usable after review | Use only if evidence is available. |

## TM Barpi Technology And Brand Claims To Verify

| Claim / topic | Evidence status | Amazon listing use status | Notes |
| --- | --- | --- | --- |
| SNECO vacuum drying | Evidence needed | Potentially usable after review | Core technology language. |
| 34-38°C in Barpi communication | Evidence needed | Not approved for listing yet | Use only after support and wording review. |
| Up to 99% moisture removal | Evidence needed | Not approved for listing yet | Quantified technical claim; needs support. |
| Up to 95% nutrient preservation | Evidence needed | Not approved for listing yet | Higher-risk quantified nutrition/technology claim; needs support. |
| Storage: 0-30°C, no refrigeration | Evidence needed | Potentially usable after review | Supports ambient positioning if documents confirm. |
| Shelf life: up to 12 months without added preservatives | Evidence needed | Potentially usable after review | Needs SKU-level shelf-life evidence. |
| Crunchy, light texture | Evidence needed | Potentially usable after review | Verify SKU-specific texture. |
| Natural aroma without artificial flavoring | Evidence needed | Potentially usable after review | Verify formulation and compliant wording. |

## Open Questions And Notes

- Do not use old Best Before and Batch Number from sticker samples.
- Confirm final production batch and best-before data only when new production is ready.
- Confirm whether "Keto friendly" and "Healthy crunchy supersnack" should be excluded from Amazon listing copy.
- Confirm exact Amazon-compliant wording for "From 100% cheese".
- Confirm support documents for renewable energy and SIAL Innovation claim.
- Confirm exact certificate wording and logo/award-use rights for SIAL Innovation Paris 2024 Grand Prix.
- Confirm final Amazon-compliant local language wording for DE, FR, IT, ES.
- Confirm whether "High protein" can be used in title, bullets, and A+ after nutrition/spec verification.

## Current Status

TM Barpi claims intake is active. SnEco claims work is paused until production price, packaging, and unit economics inputs are ready.
